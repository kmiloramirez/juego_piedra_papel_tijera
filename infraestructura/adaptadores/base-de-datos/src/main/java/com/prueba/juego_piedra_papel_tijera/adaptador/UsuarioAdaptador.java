...
public class UsuarioAdaptador implements UsuarioRepositorio {
    ...
    public Optional<Usuario> buscarUsuarioPorNombre(String nombreUsuario) {
        return usuarioRepositorio.findByNombre(nombreUsuario).stream().map(UsuarioMapeadorBaseDeDatos::convertirAModelo).findFirst();
    }

    @Override
    public Optional<Usuario> buscarUsuarioPorIdentificacion(Integer identificacion) {
        return usuarioRepositorio.findById(identificacion.longValue()).stream().map(UsuarioMapeadorBaseDeDatos::convertirAModelo).findFirst();    }
}
